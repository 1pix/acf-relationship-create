(function (
    ...
    acf.addAction("prepare", function () {
            ...
            bazalt_rc_open_tooltip: function (e) {
                ...
                if ($tooltip.find("ul li").length > 1) {
                    ...
                    if (context_media_modal) {
                        $media_modal
                            .find("div.media-frame-content div.settings")
                            .prepend($tooltip);

                        // We need to know if we are in a repeater field
                        var $repeater_parent_cell = that.$el.parents(
                            "td.acf-field:first"
                        );

                        var tooltip_pos_top =
                            $repeater_parent_cell.length == 1
                                ? that.$el
                                      .parents("td.acf-field:first")
                                      .position().top
                                : 0;
                        tooltip_pos_top +=
                            that.$el.position().top -
                            $tooltip.height() -
                            6 +
                            (that.$el.outerHeight(true) -
                                that.$el.innerHeight());

                        var tooltip_pos_left =
                            $repeater_parent_cell.length == 1
                                ? that.$el
                                      .parents("td.acf-field:first")
                                      .position().left
                                : 0;
                        tooltip_pos_left +=
                            that.$el.position().left - $tooltip.width() / 2;
                        tooltip_pos_left +=
                            $repeater_parent_cell.length == 1
                                ? that.$el.outerWidth(true)
                                : that.$el.outerWidth(true) / 2;
            ...
            bazalt_rc_open_lightbox: function (e, $el) {
                ...
                if ($media_modal.length === 1) {
                    ...
                    url = url.replace(
                        ...
                        $media_modal
                            .find(
                                "div.media-frame-content div.attachment-details"
                            )
                            .attr("data-id")
                    );
                } else {
        ...
        $.extend(
            acf.models.RelationshipField.prototype,
            model_prototype_extension
        );
        $.extend(
            acf.models.PostObjectField.prototype,
            model_prototype_extension
        );

        var model_events_extensions = {
            "click .acf-relationship-create-link":
                "bazalt_rc_trigger_open_tooltip",
            bazalt_rc_on_post_created: "bazalt_rc_on_post_created",
        };
        $.extend(
            acf.models.RelationshipField.prototype.events,
            model_events_extensions
        );
        $.extend(
            acf.models.PostObjectField.prototype.events,
            model_events_extensions
        );
    });
     ...
     */
    $(document).on("acf-relationship-create/created", function (
        e,
        field_uniq_id,
        post_data
    ) {
        ...
        switch ($field.attr("data-type")) {
            ...
            case "post_object":
                ...
                if (new_select2_value.text === "")
                    new_select2_value.text =
                        acf_relationship_create_pro_field.i18n.no_title;

                // Create <option> if not exists
                var $option = $select.find(
                    'option[value="' + new_select2_value.id + '"]'
                );
                if ($option.length < 1)
                    $select.append(
                        $("<option></option>")
                            .attr("value", new_select2_value.id)
                            .text(new_select2_value.text)
                    );
                ...
                if ($select.prop("multiple")) {
                    ...
                    $.each(select2_data, function (index, select2_single_data) {
                        // Create <option> if not exists
                        var $option = $select.find(
                            'option[value="' + select2_single_data.id + '"]'
                        );
                        if ($option.length < 1)
                            $select.append(
                                $("<option></option>")
                                    .attr("value", select2_single_data.id)
                                    .text(select2_single_data.text)
                            );
                    ...
                    $select
                        .val(new_select2_value.id.toString())
                        .trigger("change");
                }
                break;
        }
    ...
    $(document).ready(function () {
        ...
        function on_acf_relationship_field_ready($el, field_type) {
            ...
            $el.attr(
                "data-acf-rc-uniqid",
                acf_relationship_create.generate_random_id()
            );
    ...
    window.acf_relationship_create_pro_field || {}
);
